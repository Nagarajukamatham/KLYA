...
-(void)drawRect:(CGRect)rect{
    ...
    //  colorWithRed:0.64 green:0.33 blue:0.41 alpha:1.0];
    const CGFloat *color=CGColorGetComponents(alertErrorColor.CGColor);
    ...
    // Here you change Y value to move the error view down
    CGRect imgframe=CGRectMake((showOnRect.origin.x+((showOnRect.size.width-30)/2)), ((showOnRect.size.height/2)+showOnRect.origin.y)+7, 30, 13);
    
    // to remove triangle image use this image
...
//    view.layer.shadowOffset=CGSizeMake(0, 0);
    ...
    header.backgroundColor = alertErrorColor;
    [view addSubview:header];
    header.translatesAutoresizingMaskIntoConstraints=NO;
    NSDictionary *headerdic = @{@"header":header};
    
    // header lbl width
    NSLayoutConstraint *widthConstraint = [NSLayoutConstraint constraintWithItem:header
    ...
    //Header lable height
    NSArray *header_V = [NSLayoutConstraint constraintsWithVisualFormat:@"V:[header(3)]" options:0 metrics:Nil views:headerdic];
    [header addConstraints:header_V];
    
    // X - position
    NSArray *header_posH = [NSLayoutConstraint constraintsWithVisualFormat:@"H:|-0-[header]" options:0 metrics:Nil views:headerdic];
    
    // Y - position
    NSArray *header_posV = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-0-[header]" options:0 metrics:Nil views:headerdic];
    [view addConstraints:header_posH];
    [view addConstraints:header_posV];
...
-(UIColor*)colorHexStr:(NSString*)hex
{
